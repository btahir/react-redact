...
> **DOM note:** for `blur`/`mask`/`replace`, every auto-detected match is wrapped in a span
> carrying a `data-redact-original` attribute with the real matched text (used to restore the page
> when redaction toggles off). That attribute is present in the DOM — including in mask/replace
> mode — so it's inspectable via dev tools. **`mode="secure"` is the exception:** it never writes
> `data-redact-original` (or the real value in any other DOM form); the original is kept only in an
> in-memory `WeakMap` for restoration, which the `MutationObserver` rescan path also respects. See
> the [Security model](/docs#security-model) note and [Modes → Secure](/docs/modes#secure).
