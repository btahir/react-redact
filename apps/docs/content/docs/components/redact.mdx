...
- **secure** — Same display logic as replace, but with a stronger guarantee: `children` is never
  rendered while enabled, so the real value never becomes a DOM text node. See
  [Modes → Secure](/docs/modes#secure).
...
> Mask/replace/secure need plain text to compute a bullet count or fake value. Passing a React
> element (rather than a string) as `children` falls back to a fixed-length placeholder and logs
> a dev-only console warning — use `mode="blur"` or `mode="custom"` for non-text children.
