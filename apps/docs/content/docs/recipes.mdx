...
Use URL param or env to start in redacted state (e.g. permanent demo env).

```tsx
const initialRedact =
  typeof window !== 'undefined'
    ? new URLSearchParams(window.location.search).get('redact') === 'true'
    : process.env.NEXT_PUBLIC_REDACT === 'true';

<RedactProvider enabled={initialRedact} mode="replace">
  <Dashboard />
</RedactProvider>
```
